## 👋️ About Me

I am a Research Associate at Heinrich Heine University Düsseldorf, a researcher at the Heine Center for Artificial Intelligence and Data Science (HeiCAD), and a Visiting Researcher at GESIS – Leibniz Institute for the Social Sciences.

My research focuses on Artificial Intelligence (AI), Natural Language Processing (NLP), Large Language Models (LLMs), and Generative AI (GenAI). I am particularly interested in how AI systems can learn to reason, understand language, and generalize across domains. My work spans computational argumentation, social media analysis, Conversational AI, and the development of intelligent systems capable of human-like reasoning and language understanding.

With more than five years of research experience, I have developed and evaluated models for NLP and speech-related applications.

My research has been published at leading venues such as ACL, NAACL, and LREC-COLING, and I actively contribute to the community as a reviewer for top-tier NLP conferences.

Beyond research, I enjoy mentoring students, developing educational content, and teaching Python programming and AI-related topics to make technology more accessible to broader audiences.

My long-term research vision is to contribute to the development of AI systems that move beyond pattern matching toward human-like reasoning, language abilities, and generalization across diverse domains and tasks.

---

## 🎓 Education

**Ph.D. in Computer Science**  
*2021–2026 | Heinrich Heine University Düsseldorf*  
**Final Grade:** 0.5 (*Summa Cum Laude ≈ GPA 4.0 of 4.0*)

**M.Sc. in Computer Science**  
*2019–2021 | Heinrich Heine University Düsseldorf*  
**Final Grade:** 1.1 (*Very Good ≈ GPA 3.7 - 4.0 of 4.0*)

**B.Sc. in Computer Science**  
*2016–2019 | Heinrich Heine University Düsseldorf*  
**Final Grade:** 2.5 (*Good ≈ GPA 3.0 – 3.3 of 4.0*)

---

## 💼 Professional Experience

**Research Associate**  
*2021–Present* | *Heinrich Heine University Düsseldorf*  
- Conducted research in Artificial Intelligence, Natural Language Processing, Generative AI, and Conversational AI
- Contributed to a DFG-funded international AI research project
- Supervised and mentored Bachelor's, Master's, and Ph.D. students
- Developed educational content for the e-learning initiative *AI for All*
- Lecturer: *Computer Science for Non-Computer Scientists*

**Research Assistant**  
*2019–2021* | *Heinrich Heine University Düsseldorf*  
- Developed Python-based research software and data analysis tools
- Lecturer: *Computer Science for Non-Computer Scientists*

**Student Research Assistant**  
*2016–2019* | *Heinrich Heine University Düsseldorf*  
- Developed Python-based online conversation platforms for research
- Teaching Assistant: *Professional Software Development with Java*
- Teaching Assistant: *C and NASM Assembler*

---

## 🤓 Committee Lead
- Feger, M., Boland, K., Romberg, J., & Dietze, S. [*Generalizability of Argument Identification in Context*](https://touche.webis.de/clef26/touche26-web/generalizable-argument-mining.html). CLEF 2026.

---

## 📚 Publications

🎓 **Dissertation**
- Feger, M. [*The Limits of Progress: Understanding the Performance of State-of-the-Art Language Models in Argument Mining*](https://docserv.uni-duesseldorf.de/servlets/DocumentServlet?id=73263). ULB 2026.

🥇 **Top-Tier Conferences & Journals**
- Kiesel, J., Feger, M., Hagen, T., Heineking, S., Heinrich, M., Fröbe, M., Boland, K., Pertsch, W., Romberg, J., Zelch, I., Dietze, S., Hagen, M., Potthast, M., & Stein, B. [*Overview of Touché 2026: Argumentation Systems*](https://doi.org/10.1007/978-3-032-21321-1_38). ECIR 2026.
- Feger, M., Boland, K., & Dietze, S. [*Limited Generalizability in Argument Mining*](https://aclanthology.org/2025.acl-long.1164/). ACL 2025.
- Weber, S., Feger, M., & Pilgermann, M. [*A Unified Evaluation Approach for LLM Honeypots*](https://doi.org/10.1109/ACCESS.2024.3472460). IEEE Access (2024).
- Feger, M., & Dietze, S. [*BERTweet’s TACO Fiesta: Argument Mining on Twitter*](https://doi.org/10.18653/v1/2024.findings-naacl.146). NAACL 2024.
- Feger, M., & Dietze, S. [*TACO — Twitter Arguments from Conversations*](https://aclanthology.org/2024.lrec-main.1349/). LREC-COLING 2024.

🥈 **Other Peer-Reviewed Publications**  
- Braun, M., & Feger, M. [*TextLabel: A Web App for Text Annotation*](https://link.springer.com/chapter/10.1007/978-3-031-94171-9_21). HCII 2025.
- Zelle, Y., Grison, T., & Feger, M. [*SciTok – Web Scraping Tool for Social Science*](https://doi.org/10.1007/978-3-031-49212-9_14). HCII 2023.
- Steimann, J., Feger, M., & Mauve, M. [*Inspiring Perspectives in Comment Sections*](https://doi.org/10.1007/978-3-031-06424-1_10). HCII 2022.
- Feger, M., Steimann, J., & Meter, C. [*Structure or Content? Argument Relevance*](https://doi.org/10.3233/FAIA200505). COMMA 2020.

---

## 🎓✨ Featured AI Education Videos

🌱 **Machine Learning & Sustainability Tools**

<div style="padding:56.25% 0 0 0;position:relative; text-align: center; margin-bottom: 1.2em;"><iframe src="https://player.vimeo.com/video/980543329?
h=5501d3a2bc&amp;badge=0&amp;
autopause=0&amp;player_id=0&amp;app_id=58479" frameborder="0" allow="autoplay; fullscreen; picture-in-picture; clipboard-write; encrypted-media" style="position:absolute;top:0;left:0;width:100%;height:100%;" title="05_05Transfer_Nachhaltigkeitstools"></iframe></div><script src="https://player.vimeo.com/api/player.js"></script>

🎥 **Watch More of my AI Learning Videos**

- 🤖 [*Explainable, Hybrid, and Robust AI*](https://moodle.ki-campus.org/mod/page/view.php?id=16591)
- 👩‍💻 [*Programming Neural Networks from Scratch*](https://www.youtube.com/watch?v=7-hP8FSIPLE)
- 📝 [*Text Processing with Transformers*](https://www.youtube.com/watch?v=wYVeXXVtSQ8)
- 🖼️ [*Image Processing with Convolutional Neural Networks*](https://www.youtube.com/watch?v=czh68ShwsVk)

📚 **Or Explore the Full Lecture Series**

- ☝️ [AI for All 1: Introduction to Artificial Intelligence](https://moodle.ki-campus.org/course/view.php?id=99)
- ✌️ [AI for All 2: Understanding, Evaluating, Reflecting](https://moodle.ki-campus.org/course/view.php?id=111)

---

## 🧪 Technical Skills

**💻 Programming Languages**

- Python, SQL, Java, JavaScript, HTML, C/C++, NASM

**🤖 Artificial Intelligence, Deep Learning & NLP**

- PyTorch, TensorFlow, Transformers, Hugging Face Ecosystem, Conversational AI, Information Retrieval, Speech Recognition (ASR), Whisper, Argument Mining

**🧠 Language Models & Generative AI**

- LLMs, SLMs, Foundation Models, OpenAI API, LangChain, RAG, Agentic AI & MCP, LoRA/PEFT, Prompt Engineering, Evaluation & Benchmarking, Lightning AI, Together AI, vLLM

**📊 Machine Learning & Data Science**

- scikit-learn, pandas, NumPy, SciPy, Statistical Analysis & Testing, Model Evaluation, Data Mining

**🗃️ Databases & Search**

- PostgreSQL, Neo4j, Elasticsearch, SQLite

**⚙️ Software Engineering, MLOps & AI Infrastructure**

- Git, Docker, FastAPI, Django, CI/CD, TDD, GPU-Accelerated Training & Inference

---

## 🌐 Languages

- **German** 🇩🇪 – Native
- **English** 🇬🇧 🇺🇸 – Full Professional Proficiency
- **Thai** 🇹🇭 – Conversational Proficiency